...
import morph.extensions_list_ui
...
class Test(omni.kit.test.AsyncTestCase):
    ...
    async def test_hello_public_function(self):
        result = morph.extensions_list_ui.some_public_function(4)
        self.assertEqual(result, 256)

    async def test_window_button(self):
        # Find the counter label by expected text.
        label = ui_test.find(
            "Extension List UI//Frame/**/Label[*].text=='empty'"
        )

        # Find buttons in our window
        add_button = ui_test.find(
            "Extension List UI//Frame/**/Button[*].text=='Add'"
        )
        reset_button = ui_test.find(
            "Extension List UI//Frame/**/Button[*].text=='Reset'"
        )
        ...
        label = ui_test.find(
            "Extension List UI//Frame/**/Label[*].text=='count: 1'"
        )
        self.assertEqual(label.widget.text, "count: 1")

        await add_button.click()
        label = ui_test.find(
            "Extension List UI//Frame/**/Label[*].text=='count: 2'"
        )
