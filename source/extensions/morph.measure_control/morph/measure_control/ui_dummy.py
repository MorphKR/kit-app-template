...
class DummyMeasureControlUI:
    ...
    def __init__(self, service):
        ...
        self._window = ui.Window("Measure Control (Dummy UI)", width=520, height=140, visible=False)
        self._build()
    ...
    def _build(self):
        with self._window.frame:
            with ui.VStack(spacing=8, height=0):
                ...
                ui.Button("Run Mesh Measure", height=28, clicked_fn=self._on_run_mesh_clicked)
                with ui.HStack(height=24):
                    ui.Label("Status", width=50)
                    self._ro_string(self._m_status, width=450)
